...
function PlotCopyMainPage:DoInit()
  ...
  if not self.m_ChapterPlotData then
    self.m_ChapterPlotData = self:LoadChapterPlotTypeConfigData()
  end
...
function PlotCopyMainPage:LoadChapterPlotTypeConfigData()
  local data = configManager.GetData("config_chapter_plot_type")
  return data
end
