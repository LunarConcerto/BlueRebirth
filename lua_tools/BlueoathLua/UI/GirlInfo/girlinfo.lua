...
local tabUIs = {
  ...
  {
    icon = "uipic_ui_toggle_im_pingjia",
    txt = UIHelper.GetString(920000234)
  }
}
...
function GirlInfo:__LoadRightPage(nindex, HeroId)
  ...
  if pageIndex == 5 then
    local tabHeroInfo = Data.heroData:GetHeroById(HeroId)
    local shipInfoId = configManager.GetDataById("config_ship_main", tabHeroInfo.TemplateId).ship_info_id
    HeroId = configManager.GetDataById("config_ship_info", shipInfoId).sf_id
  end
...
function GirlInfo:__SwitchTogs(index)
  ...
  self.toggleParts[realIndex].isOn = true
