...
function RedDotLogic.PlotById(plotId)
  local isPass = Logic.copyLogic:IsChapterPassByChapterId(plotId)
  return not isPass
end

function RedDotLogic.MianPlot(classId)
  local chapterId, copyId, isPass = Logic.copyLogic:GetLastChapterAndCopyById(classId)
  local plotCopyInfo = Data.copyData:GetPlotCopyDataCopyId(copyId)
  local plot_IsOpen = Logic.copyLogic:CheckPlotClassCopyIsOpen(classId)
  local plotChapterIsOpen = Logic.copyLogic:_CheckPlotCopyIsOpen(chapterId)
  if isPass or plot_IsOpen == false or not plotChapterIsOpen then
    return false
  elseif not plotCopyInfo or plotCopyInfo.FirstPassTime == 0 then
    return true
  else
    return false
  end
