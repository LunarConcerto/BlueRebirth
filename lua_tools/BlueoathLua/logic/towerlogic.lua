...
function TowerLogic:IfNeedEquipTransplant(fleetType)
  ...
  for i, heroId in ipairs(fleetDataTower) do
    ...
    for i = 1, #normalShipEquips do
      if normalShipEquips[i].state == MEquipState.LOCK then
        if towerShipEquips[i].EquipsId == normalShipEquips[i].EquipsId and towerShipEquips[i].state == normalShipEquips[i].state then
          needTrans = false
        else
          needTrans = true
          break
